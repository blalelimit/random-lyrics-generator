// const lyrics = ["\"I don't wanna look at anything else now that I saw you.\" (Daylight)",
...
async function getLyrics() {
	...
	const lyrics = (await response.text()).split('\n');
	return lyrics;
}

async function newQuote() {
	const lyrics = await getLyrics();
	let val = Math.floor(Math.random()*lyrics.length);
	console.log(val);
	document.getElementById('displayQuote').innerHTML = lyrics[val];
};
